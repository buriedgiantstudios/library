...
export class CardsService {
  ...
  public init() {
    ...
    if (environment.localCDNUrl) {
      const obs = this.http
        .get(`${environment.localCDNUrl}/cards.min.json`)
        .pipe(
          catchError(() =>
            this.http.get(`${environment.baseUrl}/cards.min.json`),
          ),
        );
  ...
  // card utilities
  private reformatCardsWithErrataAndFAQ: Signal<ICard[]> = computed(() =>
  ...
  public getCardByIdOrName(codeOrName: string): ICard | undefined {
    return (
      this.cardsById()[codeOrName] ??
      this.cardsByName()[codeOrName] ??
      undefined
    );
  }
