...
1. Figure out where you want to dump your content. It should be either a fork of [this repository](https://github.com/BuriedGiantStudios/cards), or at minimum the output should be formatted the same as that repository.
1. Change `https://cardcdn.buriedgiantstudios.com` to your desired URL. At this time, it can't be changed easily in one place, so a find+replace all will be necessary. The `cards` repository will also need a [script update](https://github.com/BuriedGiantStudios/cards/blob/master/scripts/build-card-data.ts#L20) to change the base URL of the images.
